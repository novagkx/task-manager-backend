...
class TaskController {
    async createTask(req, res) {
        ...
        await pool.query(`INSERT INTO tasks (title, description, priority, status, deadline, today) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [
            title,
            description || null,
            priority || null,
            status,
    ...
    async updateTask(req, res) {
        const { id, title, description, priority, status, deadline, today } = req.body;
        await pool.query(`UPDATE tasks set title = $2, description = $3, priority = $4, status = $5, deadline = $6, today = $7 WHERE id = $1 RETURNING *`, [id, title, description, priority, status, deadline, today]);
        res
            .status(200)
